...
INSERT INTO `activity_engine`.`t_table_field_detail`(`table_field_id`, `name`, `code`, `is_default_checked`, `is_allow_uncheck`, `is_default_top`, `is_allow_top`, `is_sortable`, `min_width`, `align`, `sequence`) SELECT a.id, '学号', 'studentNo', 1, 1, 0, 1, 1, 120, 'left', 8 FROM `activity_engine`.`t_table_field` a WHERE a.type = 'activity_inspection_manage' and a.associated_type = 'activity';
INSERT INTO `activity_engine`.`t_table_field_detail`(`table_field_id`, `name`, `code`, `is_default_checked`, `is_allow_uncheck`, `is_default_top`, `is_allow_top`, `is_sortable`, `min_width`, `align`, `sequence`) SELECT a.id, '手机号', 'mobile', 1, 1, 0, 1, 1, 120, 'left', 9 FROM `activity_engine`.`t_table_field` a WHERE a.type = 'activity_inspection_manage' and a.associated_type = 'activity';

-- 活动管理字段

INSERT INTO `activity_engine`.`t_table_field_detail`(`table_field_id`, `name`, `code`, `is_default_checked`, `is_allow_uncheck`, `is_default_top`, `is_allow_top`, `is_sortable`, `min_width`, `align`, `sequence`) SELECT a.id, '开始时间', 'startTime', 1, 1, 1, 1, 1, 120, 'left', 10 FROM `activity_engine`.`t_table_field` a WHERE a.type = 'activity_manage_list' and a.associated_type = 'activity_market' ;
INSERT INTO `activity_engine`.`t_table_field_detail`(`table_field_id`, `name`, `code`, `is_default_checked`, `is_allow_uncheck`, `is_default_top`, `is_allow_top`, `is_sortable`, `min_width`, `align`, `sequence`) SELECT a.id, '结束时间', 'endTime', 1, 1, 1, 1, 1, 120, 'left', 11 FROM `activity_engine`.`t_table_field` a WHERE a.type = 'activity_manage_list' and a.associated_type = 'activity_market' ;

UPDATE `activity_engine`.`t_table_field_detail` SET `is_deleted` = 1 WHERE  `name` = '活动时间' and `code` = 'activityStartEndTime' and `table_field_id` = (SELECT a.id FROM `activity_engine`.`t_table_field` a WHERE a.type = 'activity_manage_list' and a.associated_type = 'activity_market');