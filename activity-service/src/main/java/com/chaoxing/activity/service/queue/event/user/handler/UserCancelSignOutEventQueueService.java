 ...
 * @author wwb
 * @version ver 1.0
 * @className UserCancelSignOutEventQueueService
 * @description
 * @blame wwb
 * @date 2021-10-28 17:56:36
 */
@Slf4j
@Service
public class UserCancelSignOutEventQueueService {
    ...
    public void handle(UserCancelSignOutEventOrigin eventOrigin) {
        if (eventOrigin == null) {
            return;
        }
        ...
        // 记录用户行为
        UserActionRecordQueue.QueueParamDTO queueParam = new UserActionRecordQueue.QueueParamDTO(uid, activityId, UserActionTypeEnum.SIGN_OUT, UserActionEnum.CANCEL_SIGNED_OUT, String.valueOf(eventOrigin.getSignOutId()), DateUtils.timestamp2Date(eventOrigin.getTimestamp()));
