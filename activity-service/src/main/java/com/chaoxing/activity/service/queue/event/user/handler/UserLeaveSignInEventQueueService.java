 ...
 * @author wwb
 * @version ver 1.0
 * @className UserLeaveSignInEventQueueService
 * @description
 * @blame wwb
 * @date 2021-10-28 18:12:35
 */
@Slf4j
@Service
public class UserLeaveSignInEventQueueService {
    ...
    public void handle(UserLeaveSignInEventOrigin eventOrigin) {
        if (eventOrigin == null) {
            return;
        }
        ...
        // 记录用户行为
        UserActionQueue.QueueParamDTO queueParam = new UserActionQueue.QueueParamDTO(uid, activityId, UserActionTypeEnum.SIGN_IN, UserActionEnum.LEAVE_SIGNED_IN, String.valueOf(eventOrigin.getSignInId()), DateUtils.timestamp2Date(eventOrigin.getTimestamp()));
