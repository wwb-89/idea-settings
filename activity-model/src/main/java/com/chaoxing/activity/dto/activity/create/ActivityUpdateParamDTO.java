	...
	*/
	...
	public static ActivityUpdateParamDTO buildActivityUpdateParam(Activity activity, ActivityCreateParamDTO waitUpdateInfo) {
		...
		return ActivityUpdateParamDTO.builder()
				.id(activity.getId())
				.name(Optional.ofNullable(waitUpdateInfo.getName()).filter(StringUtils::isNotBlank).orElse(activity.getName()))
				...
				.organisers(Optional.ofNullable(waitUpdateInfo.getOrganisers()).filter(StringUtils::isNotBlank).orElse(activity.getOrganisers()))
				.activityType(activity.getActivityType())
				.address(Optional.ofNullable(waitUpdateInfo.getAddress()).filter(StringUtils::isNotBlank).orElse(activity.getAddress()))
				.detailAddress(Optional.ofNullable(waitUpdateInfo.getDetailAddress()).filter(StringUtils::isNotBlank).orElse(activity.getDetailAddress()))
